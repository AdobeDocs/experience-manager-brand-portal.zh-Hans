---
title: '配置Contribution文件夹并将其从Experience Manager资产发布到Brand Portal '
seo-title: Configure and publish contribution folder from Experience Manager Assets to Brand Portal
description: 深入了解如何配置和发布贡献文件夹，以将资产从Experience Manager到Brand Portal。
...
# 在Experience Manager资产中配置Contribution文件夹 {#configure-contribution-folder}

对于协作资产源，Experience Manager资产用户（管理员和具有权限的非管理员用户）可以创建类型为&#x200B;**资产贡献**&#x200B;的新文件夹，以确保创建的新文件夹对Brand Portal用户提交资产的操作是开放的。  这会自动触发一个工作流，该工作流会在新创建的&#x200B;**Contribution**&#x200B;文件夹中创建另外两个子文件夹，分别称为&#x200B;**SHARED**&#x200B;和&#x200B;**NEW**。

然后，Experience Manager资产用户通过将关于应添加到贡献文件夹的资产类型以及一组基准资产的简报上传到&#x200B;**SHARED**&#x200B;文件夹来定义资产要求，以确保Brand Portal用户获得所需的信息。 然后，管理员可以在将新创建的Contribution文件夹发布到Brand Portal之前，向活动的Brand Portal用户授予对Contribution文件夹的访问权限。

以下视频演示了如何在Experience Manager资产中配置Contribution文件夹：

>[!VIDEO](https://video.tv.adobe.com/v/30547)

Experience Manager资产用户在配置贡献文件夹时，会执行以下活动：
...
* [将贡献文件夹从Experience Manager资产发布到Brand Portal](#publish-contribution-folder-to-brand-portal)

## 创建贡献文件夹 {#create-contribution-folder}


Experience ManagerExperience Manager管理员和有权创建新文件夹的非管理员用户可以在资产中创建贡献文件夹。
要创建贡献文件夹，请创建一个类型为“资产贡献”的新文件夹，并确保创建的新文件夹已打开，可供Brand Portal用户提交资产。  这会自动触发一个工作流，该工作流会在贡献文件夹内创建另外两个子文件夹，称为SHARED和NEW。


>[!NOTE]
>
>您可以在一个文件夹中创建多个贡献文件夹。 请勿在另一个贡献文件夹内创建贡献文件夹。
...
1. 导航至&#x200B;**[!UICONTROL Assets]** > **[!UICONTROL 文件]**。 它列出了Experience Manager资产存储库中的所有现有文件夹。
...
1. 单击&#x200B;**[!UICONTROL 创建]**。您可以在Experience Manager资产存储库中看到列出的贡献文件夹。
...
Experience Manager资产管理员在配置贡献文件夹的属性时，会执行以下活动。
...
1. 输入贡献文件夹的高级&#x200B;**[!UICONTROL 描述]**。
1. 单击&#x200B;**[!UICONTROL 上传Brief]**&#x200B;可从本地计算机浏览并上传&#x200B;**资产要求文档**。
...
>[!NOTE]
>
>搜索结果基于在Brand Portal Assets中配置的Experience Manager列表。 确保已更新Brand Portal用户列表。
...
配置贡献文件夹后，Experience Manager资产用户（管理员/非管理员用户）可以将贡献文件夹从Experience Manager资产发布到Brand Portal。 具有访问贡献文件夹权限的Brand Portal用户将在发布操作完成时收到电子邮件/脉冲通知。
...
电子邮件/脉冲通知会发送给分配到贡献文件夹的Brand Portal用户。 Brand Portal用户可以访问贡献文件夹并开始贡献。 请参阅[将资产上传到Contribution文件夹并发布到Experience Manager资产](brand-portal-publish-contribution-folder-to-aem-assets.md)。
