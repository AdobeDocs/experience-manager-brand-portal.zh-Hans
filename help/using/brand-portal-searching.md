...
description: 通过Brand Portal搜索功能，您可以使用Omnisearch快速搜索相关资源，而搜索筛选器可帮助您进一步缩小搜索范围。 将搜索另存为智能收藏集，以供将来使用。
contentOwner: bdhar
...
通过Brand Portal搜索功能，您可以使用Omnisearch快速搜索相关资源和使用过滤器的彩块化搜索帮助您进一步缩小搜索范围。 您可以在文件或文件夹级别搜索资产，并将搜索结果另存为智能收藏集。
...
1. 在工具栏中，单击&#x200B;**[!UICONTROL 搜索]**&#x200B;图标，或按&#x200B;**[!UICONTROL /]**（正斜杠）键启动Omnisearch。
...
1. 在搜索框中，键入要搜索的资源的关键字。
   ...
   >* 在搜索`mountain biking`时，Omnisearch返回搜索结果中元数据字段中同时具有`mountain`和`biking`可用的所有资源。 例如，`Title`字段中的`mountain`和`Description`字段中的`biking`。 这两个术语必须在元数据字段中可用，才能显示在搜索结果中。 但是，Omnisearch会在搜索结果中返回资产，即使智能标记元数据字段中只有两个术语之一可用。 例如，假设某个资产将`mountain`作为智能标记，但在任何其他元数据字段中缺少`biking`。 然后搜索`mountain biking`。 Omnisearch仍会在搜索结果中返回资产。 此工作流可确保不会遗漏具有相关标记的资产。
...
要了解有关使用智能标记的资源搜索行为的更多信息，请转到[了解搜索结果和行为](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-65/content/assets/using/search-assets)。

## 在“筛选器”面板中使用Facet进行搜索 {#search-using-facets-in-filters-panel}

“筛选器”面板中的搜索Facet可为您的搜索体验添加粒度，并使搜索功能更有效。 搜索Facet使用多个维度（谓词），使您能够执行复杂的搜索。 您可以轻松地深入到所需的详细级别以进行更集中的搜索。

例如，如果要查找图像，您可以选择是要位图还是矢量图像。 可通过在“文件类型”搜索方面指定图像的MIME类型来进一步缩小搜索范围。 同样，在搜索文档时，可以指定格式，例如PDF或MS® Word格式。
...
例如，使用以下标准过滤器：
   ...
   * **[!UICONTROL 文件类型]**，用于指定您要查找的资源文件的类型（图像、文档、多媒体、存档）。 此外，您可以缩小搜索范围，例如，为文档的图像或格式（PDF或MS® Word）指定MIME类型（Tiff、位图、GIMP图像）。
   ...
   * **[!UICONTROL 视频格式]**，用于根据其格式(DVI、Flash、MPEG4、MPEG、OGG Theora、QuickTime、Windows Media、WebM)搜索视频资产。

   您可以通过编辑基础搜索表单在“筛选器”面板中使用[自定义搜索彩块化](../using/brand-portal-search-facets.md)。

   * **[!UICONTROL 属性谓词]**（如果在搜索表单中使用）允许您搜索与谓词所映射到的元数据属性匹配的资源。\
     例如，如果属性谓词映射到`jcr:content/metadata/dc:title`，则可以根据资产的标题搜索资产。\
...
要允许使用属性谓词中的部分短语进行资产搜索，请在“搜索表单”中启用**[!UICONTROL 部分搜索]**&#x200B;复选框。 通过此方法，您可以搜索所需的资源，即使您未指定资源元数据中使用的确切单词或短语。
     ...
     您可以：
      * 在“过滤器”面板的Facet中，指定出现在搜索短语中的单词。 例如，如果您搜索术语&#x200B;**climb**（并且属性谓词映射到`dc:title`属性），则返回其标题短语中包含单词&#x200B;**climb**&#x200B;的所有资产。
      * 指定搜索短语中出现的单词的一部分，并使用通配符(&#42;)填补空白。
例如，搜索：
...
1. 单击&#x200B;**[!UICONTROL 保存智能收藏集]**&#x200B;并提供智能收藏集的名称。

   要使所有用户均可访问智能收藏集，请选择&#x200B;**[!UICONTROL 公共]**。 将显示一条消息，确认智能收藏集已创建并已添加到保存的搜索列表。

   >[!NOTE]
   >
   >您可以限制非管理员用户公开智能收藏集，以避免非管理员用户在组织的Brand Portal上创建大量公共智能收藏集。 组织可以禁用管理工具面板中可用的&#x200B;**[!UICONTROL 常规]**&#x200B;设置中的&#x200B;**[!UICONTROL 允许公共智能收藏集创建]**&#x200B;配置。
