...
在[[!UICONTROL Admin Console]](http://adminconsole.adobe.com/enterprise/overview)中，您可以视图与您的组织关联的所有产品。 产品可以是任何Experience Cloud解决方案，如Adobe Analytics、Adobe Target或AEM品牌门户。 您必须选择AEM Brand Portal产品，并创建产品用户档案。
...
>[!NOTE]
>
>要在Brand Portal中创建组，请从Adobe[!UICONTROL Admin Console]，使用&#x200B;**[!UICONTROL 产品>产品用户档案]**，而不是&#x200B;**[!UICONTROL 用户页>用户组]**。 Adobe[!UICONTROL Admin Console]中的产品用户档案用于在Brand Portal中创建组。

## 添加用户{#add-a-user}

如果您是产品管理员，请使用Adobe[[!UICONTROL Admin Console]](http://adminconsole.adobe.com/enterprise/overview)创建用户并将其分配给产品用户档案（*以前称为产品配置*），这些在品牌门户中显示为组。 您可以使用组执行批量操作，如角色管理和资产共享。

>[!NOTE]
>
>无权访问Brand Portal的新用户可以从Brand Portal的登录屏幕请求访问权限。 有关详细信息，请参阅[请求访问品牌门户](../using/brand-portal.md#request-access-to-brand-portal)。 在通知区域收到访问请求通知后，单击相关通知，然后单击&#x200B;**[!UICONTROL 授予访问权限]**。 或者，也可以按照收到的访问请求电子邮件中的链接操作。 接下来，要通过[Adobe[!UICONTROL Admin Console]](http://adminconsole.adobe.com/enterprise/overview)添加用户，请按照以下步骤中的步骤4-7操作。
...
1. 在Admin Console中，执行以下操作之一以创建新用户：
   ...
   * 在顶部的工具栏中，单击&#x200B;**[!UICONTROL 用户]**。 在[!UICONTROL Users]页面中，默认情况下选择左边栏中的[!UICONTROL Users]。 单击&#x200B;**[!UICONTROL 添加用户]**。
...
1. 单击&#x200B;**[!UICONTROL 保存]**。将向您添加的用户发送一封欢迎电子邮件。 受邀用户可以通过单击欢迎电子邮件中的链接，然后使用[!UICONTROL Adobe ID]登录来访问品牌门户。 有关详细信息，请参阅[首次登录体验](../using/brand-portal-onboarding.md)。
...
1. 在[!UICONTROL 创建新用户档案]页面中，提供用户档案名称、显示名称、用户档案说明，并选择在将用户添加到用户档案或从中删除时，是否要通过电子邮件通知用户。

   ![创建产品用户档案](assets/admin_console_addaproductprofilecreatenewprofile.png)

1. 单击&#x200B;**[!UICONTROL 完成]**。产品配置组（例如&#x200B;**[!UICONTROL 销售组]**）将添加到Brand Portal。

   ![产品用户档案](assets/admin_console_productprofileadded.png)

## 将用户添加到产品用户档案{#add-users-to-a-product-profile}

要将用户添加到品牌门户组，请将其添加到[!UICONTROL Admin Console]中的相应产品用户档案（以前称为产品配置）。 您可以单独或批量添加用户。
...
1. 在顶部的工具栏中，单击&#x200B;**[!UICONTROL 产品]**。
1. 在[!UICONTROL 产品]页面中，默认情况下选择[!UICONTROL 产品用户档案]。 打开要向其添加用户的产品用户档案，例如[!UICONTROL 销售组]。
...
1. 要将批量用户添加到产品用户档案，请执行以下操作：
   ...
   * 单击&#x200B;**[!UICONTROL 上传]**。
   如果您将用户添加到默认的产品用户档案（即Brand Portal），则欢迎电子邮件会发送到您添加的用户的电子邮件ID。 受邀用户可以单击欢迎电子邮件中的链接，然后使用[!UICONTROL Adobe ID]登录，以访问Brand Portal。 有关详细信息，请参阅[首次登录体验](../using/brand-portal-onboarding.md)。

   添加到自定义或新产品用户档案的用户不会收到电子邮件通知。

## 为用户{#provide-administrator-privileges-to-users}提供管理员权限

您可以为Brand Portal用户提供系统管理员或产品管理员权限。 请勿提供[!UICONTROL Admin Console]中提供的其他管理权限，如产品用户档案管理员、用户组管理员和支持管理员。 要进一步了解这些角色，请参阅[管理角色](https://helpx.adobe.com/enterprise/using/admin-roles.html)。
...
1. 在[!UICONTROL Users]页面中，默认情况下选择左边栏中的[!UICONTROL Users]。 单击要向其提供管理员权限的用户的用户名。
...
1. 在[!UICONTROL 编辑管理员]页面中，选择系统管理员或产品管理员。
   ...
   >
   >Adobe建议您避免使用系统管理员角色，因为它授予组织内所有产品的管理员权限。 例如，某个组织（包括三个marketing cloud产品）的系统管理员对所有三个产品都具有整套权限。 只有系统管理员才能配置AEM Assets，以便将资产从AEM Assets发布到品牌门户。 有关详细信息，请参阅[使用Brand Portal配置AEM Assets](../using/configure-aem-assets-with-brand-portal.md)。
   >
...
* [!UICONTROL 查看器]:具有此角色的用户可以视图管理员与他们共享的文件和文件夹。查看器还可以搜索和下载资产。 但是，查看器不能与其他用户共享内容（文件、文件夹、[!UICONTROL 集合]）。
* [!UICONTROL 编辑]:具有此角色的用户具有查看器的所有权限。此外，编辑人员还可以与其他用户共享内容（文件夹、[!UICONTROL 集合]、链接）。
...
1. 在[!UICONTROL 用户角色]页面中，默认情况下选择[!UICONTROL 用户]选项卡。 对于要更改其角色的用户，从&#x200B;**[!UICONTROL 角色]**&#x200B;下拉菜单中选择&#x200B;**[!UICONTROL Editor]**&#x200B;或&#x200B;**[!UICONTROL Viewer]**。
...
管理员可以将特定权限与品牌门户上的[用户组](../using/brand-portal-adding-users.md#main-pars-title-278567577)关联。 **[!UICONTROL 用户角色]**&#x200B;页面上的&#x200B;**[!UICONTROL 组]**&#x200B;选项卡允许管理员：
...
1. 从&#x200B;**[!UICONTROL 角色]**&#x200B;下拉列表中选择适当的角色。
   ...
   允许组成员访问图像文件的原始呈现形式(.jpeg、.tiff、.png、.bmp、.gif、.pjpeg、x-portable-anymap、x-portable-bitmap、x-portable-graymap、x-pixmap、x-rgb、x-xpixmap、x-icon、image/x-photoshop、.pshop、image/vnd.adobe.p)他们从门户或共享链接下载的“访问原件&#x200B;**[!UICONTROL ”选项。]**&#x200B;默认情况下，为所有用户选择“访问原件&#x200B;**[!UICONTROL ”选项。]**&#x200B;要阻止用户组访问原始演绎版，请取消选择与该组对应的选项。
