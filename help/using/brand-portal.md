...
Brand Portal用户在 [!DNL Admin Console] 具有查看器角色的Brand Portal的访问权限。 具有此角色的用户可以登录Brand Portal并访问允许的文件夹、收藏集和资产。 用户还可以浏览、预览、下载和导出资产（原始或特定演绎版）、配置帐户设置，以及搜索资产。 以下是查看器可以执行的任务列表：
